...
public class Problem0011 {
  ...
  public static void solution() {
    ...
    for (int i = 0; i < grid.length; i++) {
      for (int j = 0; j < grid[0].length; j++) {
        ...
        if (i < grid.length - (l - 1) && j < grid[0].length - (l - 1)) {
          long p = 1;
          for (int k = 0; k < l; k++) {
            p = p * grid[i + k][j + k];
            if (max < p) {
  ...
  public static int[][] grid() {
    ...
    String[] tokens = seqString.split("[ ]");

    for (int i = 0; i < 20; i++) {
      for (int j = 0; j < 20; j++) {
        grid[i][j] = Integer.parseInt(tokens[i * 20 + j]);
      }
    }
